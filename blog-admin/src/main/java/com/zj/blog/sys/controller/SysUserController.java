...
import com.zj.blog.sys.service.ISysPermissionService;
import com.zj.blog.sys.service.ISysRoleService;
import com.zj.blog.sys.service.ISysUserService;
...
public class SysUserController extends ApiController {
    ...
    public Object getUserByPage(String keyword,
                                @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                @RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize
    ) {
        try {
            return new CommonResult().success(sysUserService.page(new Page<SysUser>(pageNum, pageSize), new QueryWrapper<SysUser>().and(ValidatorUtils.notEmpty(keyword), wrapper -> wrapper.like("id", keyword).or().like("username", keyword).or().like("nick_name", keyword).or().like("email", keyword).or().like("note", keyword))));
        } catch (Exception e) {
    ...
    @SysLog(MODULE = "sys", REMARK = "更新用户状态")
    @ApiOperation("更新用户状态")
    @PostMapping(value = "/update/showStatus")
    public Object updateShowStatus(SysUser user) {
        try {
