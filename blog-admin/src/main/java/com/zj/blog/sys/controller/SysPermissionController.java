...
import com.zj.blog.sys.service.RedisService;
...
public class SysPermissionController extends BaseController {
    @Resource
    private com.zj.blog.sys.service.ISysPermissionService ISysPermissionService;
    ...
    public Object getRoleByPage(String name, Long pid,
                                @RequestParam(value = "pageNum", defaultValue = "1") Integer pageNum,
                                @RequestParam(value = "pageSize", defaultValue = "5") Integer pageSize
    ) {
        try {
            return new CommonResult().success(ISysPermissionService.page(new Page<SysPermission>(pageNum, pageSize), new QueryWrapper<SysPermission>().eq(ValidatorUtils.notEmpty(name), "name", name).eq(ValidatorUtils.notEmpty(pid), "pid", pid)));
        } catch (Exception e) {
    ...
    @SysLog(MODULE = "sys", REMARK = "根据条件查询所有后台用户权限表列表")
    @ApiOperation("根据条件查询所有后台用户权限表列表")
    @GetMapping(value = "/permisiionList")
    @PreAuthorize("hasAuthority('sys:SysPermission:read')")
    public Object getPermisiionlist() {
        try {
            return new CommonResult().success(ISysPermissionService.list(null));
        } catch (Exception e) {
            log.error("根据条件查询所有后台用户权限表列表：%s", e.getMessage(), e);
        }
        return new CommonResult().failed();
    ...
    @SysLog(MODULE = "sys", REMARK = "更新后台用户权限表状态")
    @ApiOperation("更新后台用户权限表")
    @PostMapping(value = "update/showStatus")
    @PreAuthorize("hasAuthority('sys:SysPermission:read')")
    public Object updatePermissionStatus(@RequestParam("ids") List<Long> ids, @RequestParam("status") Integer status) {
        try {
            if (ValidatorUtils.notEmpty(ids)) {
                ...
                if (ISysPermissionService.updateBatchById(sysPermissions)) {
                    return new CommonResult().success();
                }
            }
        } catch (Exception e) {
